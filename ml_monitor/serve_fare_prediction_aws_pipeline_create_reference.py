...
mlflow.set_experiment("reference-wdc-taxi-ride-fair-prediction-s3-pipeline")
...
@task(name="Write Reference", retries=3, retry_delay_seconds=2)
def write_reference(y_pred, serving):
    # Persist the predictions
    output_filename = "reference.csv"
    output_key = f"data/ref/{output_filename}"
    bucket_name = 'mlflow-artifacts-remote-ahm-amm'
    s3 = boto3.client('s3')
    csv_buffer = io.BytesIO()
    serving["predicted_amount"] = y_pred
    serving.to_csv(csv_buffer, index=False)
    csv_buffer.seek(0)
...
def main(year, month):
    ...
    # Persist the predictions
    write_reference(y_pred, serving)
